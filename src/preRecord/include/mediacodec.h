...
//-------------------encode-------------------------
typedef void (*outFormatChangedCb)(AMediaFormat *format);
...
typedef struct MediaCodecEncoder{
	...
	uint32_t duration;//预录时间
	outFormatChangedCb _format_changed_cb;
...
int mediacodec_encoder_sttop(MediaCodecEncoder* encoder);
